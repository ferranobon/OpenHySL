...
void Substructure_InfoLocation_Init (const int Length, int *Position, int *const Descriptor, InfoLocation_t *const InfoLoc) {
    ...
    for (i = 0; i < Length; i++) {
        infog2l_(&Position[i], &ione, Descriptor, &nprow, &npcol, &myrow, &mycol, &InfoLoc->LRowIndex[i], &InfoLoc->LColIndex[i], &InfoLoc->RowProcess[i], &InfoLoc->ColProcess[i]);
    }
}
