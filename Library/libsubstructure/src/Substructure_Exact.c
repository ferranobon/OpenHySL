...
void Substructure_ExactSolutionMDOF_Init (const hysl_float_t *const Mass, const hysl_float_t *const Stiff, const int NDOF, const hysl_float_t RayM_Alpha, const hysl_float_t RayM_Beta,
        const hysl_float_t RayS_Alpha, const hysl_float_t RayS_Beta, const hysl_float_t a0, const hysl_float_t a2, const hysl_float_t a3, const hysl_float_t a6, const hysl_float_t a7,
        const char *Description, ExactSim_t *const Sub) {
    int i;
    ...
    Compute_DampingRatios_Rayleigh(RayS_Alpha, RayS_Beta, Sub->EValues.Rows, Sub->EValues.Array, Sub->Damping_Ratios.Array);
    ...
    Sub->Ray_Main.Alpha = RayM_Alpha;
    Sub->Ray_Main.Beta = RayM_Beta;
    Sub->Ray_Sub.Alpha = RayS_Alpha;
    Sub->Ray_Sub.Beta = RayS_Beta;
}

void Substructure_ExactSolutionSDOF_Init (const hysl_float_t Mass, const hysl_float_t Damp, const hysl_float_t Stiff, const hysl_float_t a0, const hysl_float_t a2, const hysl_float_t a3,
        const hysl_float_t a6, const hysl_float_t a7, const char *Description, ExactSim_t *const Sub) {
    Sub->Description = strdup(Description);
...
void Compute_DampingRatios_Rayleigh (const hysl_float_t Ray_Alpha, const hysl_float_t Ray_Beta, const int Num_DOF, const hysl_float_t *const Eigen_Values, hysl_float_t *const Damping_Ratios) {
    int i; /* A counter */
    hysl_float_t omega;
...
void Substructure_ExactSolutionMDOF (const hysl_float_t DispTdT, const hysl_float_t ramp, const hysl_float_t GAcc, const hysl_float_t DeltaT, ExactSim_t *const Sub, hysl_float_t *const fc) {
    int i;
...
#else
    Sub->AccTdT = (1.0 - ramp) * (-Sub->a0 * Sub->Disp0 - Sub->a2 * Sub->Vel0 - Sub->a3 * Sub->Acc0) + ramp * (-Sub->a0 * Sub->DispT - Sub->a2 * Sub->VelT - Sub->a3 * Sub->AccT) + Sub->a0 * DispTdT;
    ...
    Duhamel_Integral(&Sub->Mass, &Sub->EValues, &Sub->EVectors, &Sub->Damping_Ratios, &Sub->Init_Disp, &Sub->Init_Vel, &Sub->Load, &Sub->End_Disp, &Sub->End_Vel, &Sub->End_Acc, DeltaT);
...
void Substructure_ExactSolutionSDOF (const hysl_float_t DispTdT, const hysl_float_t ramp, const hysl_float_t GAcc, const hysl_float_t DeltaT, ExactSim_t *const Sub, hysl_float_t *const fc) {
...
void Duhamel_Integral (const MatrixVector_t *const Mass, const MatrixVector_t *const Eigen_Values, const MatrixVector_t *const Eigen_Vectors, const MatrixVector_t *const Damping_Ratios,
        const MatrixVector_t *const Init_Disp, const MatrixVector_t *const Init_Vel, const MatrixVector_t *const Load, MatrixVector_t *const End_Disp, MatrixVector_t *const End_Vel,
        MatrixVector_t *const End_Acc, const hysl_float_t Delta_T) {
    ...
    trans = 'T';
    hysl_gemv(&trans, &temp1.Rows, &temp1.Rows, &Alpha, Eigen_Vectors->Array, &temp1.Rows, temp1.Array, &incx, &Beta, modal_disp.Array, &incy);
    hysl_gemv(&trans, &temp2.Rows, &temp1.Rows, &Alpha, Eigen_Vectors->Array, &temp2.Rows, temp2.Array, &incx, &Beta, modal_velo.Array, &incy);
    hysl_gemv(&trans, &temp1.Rows, &temp1.Rows, &Alpha, Eigen_Vectors->Array, &temp1.Rows, Load->Array, &incx, &Beta, modal_force.Array, &incy);
    ...
    trans = 'N';
    hysl_gemv(&trans, &modal_disp.Rows, &modal_disp.Rows, &Alpha, Eigen_Vectors->Array, &modal_disp.Rows, modal_disp.Array, &incx, &Beta, End_Disp->Array, &incy);
    hysl_gemv(&trans, &modal_velo.Rows, &modal_velo.Rows, &Alpha, Eigen_Vectors->Array, &modal_velo.Rows, modal_velo.Array, &incx, &Beta, End_Vel->Array, &incy);
    hysl_gemv(&trans, &modal_force.Rows, &modal_force.Rows, &Alpha, Eigen_Vectors->Array, &modal_force.Rows, modal_force.Array, &incx, &Beta, End_Acc->Array, &incy);
