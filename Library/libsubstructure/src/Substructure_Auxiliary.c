...
void Substructure_JoinNonCouplingPart (MatrixVector_t *const VecTdT_m, const MatrixVector_t *const Gain_m, const MatrixVector_t *const fcprevsub, const CouplingNode_t *const CNodes,
    ...
    hysl_gemv(&trans, &Rows, &Cols, &Alpha, Gain_m->Array, &lda, fcprevsub->Array, &incx, &Beta, VecTdT_m->Array, &incy);
...
void Substructure_MatrixXcm (const MatrixVector_t *const Mat, const CouplingNode_t *const CNodes, MatrixVector_t *const MatXcm) {
    ...
    for (icoup = 1; icoup < CNodes->Order; icoup++) {
        ...
        for (jcoup = icoup; jcoup < CNodes->Order; jcoup++) {
            ...
            hysl_copy(&Length, &Mat->Array[(CNodes->Array[jcoup] - 1) + (CNodes->Array[icoup - 1]) * Mat->Rows], &incx, &MatXcm->Array[PosXcm], &incy);
        }
    ...
    for (jcoup = CNodes->Order - 1; jcoup >= 0; jcoup = jcoup - 1) {
        ...
        hysl_copy(&Length, &Mat->Array[(CNodes->Array[jcoup] - 1) * Mat->Rows + (CNodes->Array[CNodes->Order - 1])], &incx, &MatXcm->Array[PosXcm], &incy);
    }
    Acumulated_Length = Length;
    incx = 1;
    for (icoup = CNodes->Order - 2; icoup >= 0; icoup = icoup - 1) {
        ...
        for (jcoup = icoup; jcoup >= 0; jcoup = jcoup - 1) {
            ...
            hysl_copy(&Length, &Mat->Array[(CNodes->Array[jcoup] - 1) * Mat->Rows + (CNodes->Array[icoup])], &incx, &MatXcm->Array[PosXcm], &incy);
        }
        Acumulated_Length = Acumulated_Length + Length;
    }
}
