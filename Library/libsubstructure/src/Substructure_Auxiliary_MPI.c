...
void Substructure_JoinNonCouplingPart_MPI (PMatrixVector_t *const VecTdT_m, PMatrixVector_t *const Gain_m, PMatrixVector_t *const fcprevsub, const CouplingNode_t *const CNodes,
        PMatrixVector_t *const VecTdT) {
    ...
    hysl_pgemv(&trans, &Rows, &Cols, &Alpha, Gain_m->Array, &ione, &ione, Gain_m->Desc, fcprevsub->Array, &ione, &ione, fcprevsub->Desc, &incx, &Beta, VecTdT_m->Array, &ione, &ione, VecTdT_m->Desc,
            &incy);
    ...
    for (icoup = 0; icoup < CNodes->Order; icoup++) {
        ...
        hysl_pcopy(&Length, VecTdT_m->Array, &PosXm_Row, &PosXm_Col, VecTdT_m->Desc, &incx, VecTdT->Array, &PosX_Row, &PosX_Col, VecTdT->Desc, &incy);
...
void Substructure_MatrixXc_MPI (const MPI_Comm Comm, const CouplingNode_t *const CNodes, PMatrixVector_t *const Mat, MatrixVector_t *const MatCouple) {
        ...
        for (jcoup = icoup; jcoup < CNodes->Order; jcoup++) {
            ...
            if (Cblacs_pnum(Mat->Desc[1], RowProcess, ColProcess) == 0 && myrow == RowProcess && mycol == ColProcess) {
                ...
                if (myrow == RowProcess && mycol == ColProcess) {
                    MPI_Send(&Mat->Array[(LRowIndex - 1) + Mat->LocalSize.Row * (LColIndex - 1)], 1, MPI_HYSL_FLOAT, 0, MATRIX_XC, Comm);
                }

                if (rank == 0) {
                    /* Store the diagonal elements */
                    MPI_Recv(&MatCouple->Array[icoup * CNodes->Order + jcoup], 1, MPI_HYSL_FLOAT, Cblacs_pnum(Mat->Desc[1], RowProcess, ColProcess), MATRIX_XC, Comm, &status);
...
void Substructure_MatrixXcm_MPI (const MPI_Comm Comm, PMatrixVector_t *const Mat, const CouplingNode_t *const CNodes, PMatrixVector_t *const MatXcm) {
    ...
    for (icoup = 0; icoup < CNodes->Order; icoup++) {
        ...
        PosXcm_Rows = 1;
        PosXcm_Cols = icoup + 1;
        hysl_pcopy(&Length, Mat->Array, &PosX_Rows, &PosX_Cols, Mat->Desc, &incx, MatXcm->Array, &PosXcm_Rows, &PosXcm_Cols, MatXcm->Desc, &incy);
    }
...
void Substructure_VectorXm_MPI (PMatrixVector_t *const VectorX, const CouplingNode_t *const CNodes, PMatrixVector_t *const VectorXm) {
    ...
    for (icoup = 0; icoup < CNodes->Order; icoup++) {
        ...
        /* Copy the part of the vector between two positions */
        hysl_pcopy(&Length, VectorX->Array, &PosX_Row, &PosX_Col, VectorX->Desc, &incx, VectorXm->Array, &PosXm_Row, &PosXm_Col, VectorXm->Desc, &incy);
...
void Substructure_VectorXc_MPI (const MPI_Comm Comm, PMatrixVector_t *const VecX, const CouplingNode_t *const CNodes, MatrixVector_t *const VecXc) {
    int icoup;
    int nprow, npcol, myrow, mycol;
    int rank;
    int GRowIndex, GColIndex;
    ...
    for (icoup = 0; icoup < CNodes->Order; icoup++) {
        ...
        if (Cblacs_pnum(VecX->Desc[1], RowProcess, ColProcess) == 0 && myrow == RowProcess && mycol == ColProcess) {
            ...
            if (myrow == RowProcess && mycol == ColProcess) {
                ...
                MPI_Send(&VecX->Array[(LRowIndex - 1) * VecX->LocalSize.Col + (LColIndex - 1)], 1, MPI_HYSL_FLOAT, 0, VECTOR_XC, Comm);
            }

            if (rank == 0) {
                /* Store the diagonal elements */
                MPI_Recv(&VecXc->Array[icoup], 1, MPI_HYSL_FLOAT, Cblacs_pnum(VecX->Desc[1], RowProcess, ColProcess), VECTOR_XC, Comm, &status);
            }
        }
    }
}
