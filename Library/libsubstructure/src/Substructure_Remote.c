...
void Substructure_Remote_Init (const char *RemoteType, const char *IPAddress, const char *Port, const char *Acc_Name, const char *Acc_Password, const unsigned int NSub, const int *const DOF,
...
void Substructure_Remote_SetupClientSocket (Remote_t *const RemoteNode) {
    ...
    addrCriteria.ai_family = AF_UNSPEC; /* Set the value to accept any type of addresses */
    if (RemoteNode->Type == REMOTE_TCP || RemoteNode->Type == REMOTE_NSEP || RemoteNode->Type == REMOTE_OF || RemoteNode->Type == REMOTE_CELESTINA) {
