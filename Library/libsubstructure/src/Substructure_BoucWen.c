...
void Substructure_BoucWen_Init (const hysl_float_t alpha, const hysl_float_t ko, const hysl_float_t Fy, const hysl_float_t beta, const hysl_float_t gamma, const hysl_float_t n, const hysl_float_t A0,
        const hysl_float_t deltaA, const hysl_float_t nu0, const hysl_float_t deltaNu, const hysl_float_t eta0, const hysl_float_t deltaEta, const hysl_float_t vs0, const hysl_float_t p,
        const hysl_float_t q, const hysl_float_t lambda, const hysl_float_t psi0, const hysl_float_t deltaPsi, const int BoucWen_Type, const char *Description, BoucWen_t *const Sub) {
...
void Substructure_BoucWenSurface_Init (const hysl_float_t alpha, const hysl_float_t ko, const hysl_float_t Fy, const hysl_float_t beta, const hysl_float_t gamma, const hysl_float_t n,
        const hysl_float_t A0, const hysl_float_t deltaA, const hysl_float_t nu0, const hysl_float_t deltaNu, const hysl_float_t eta0, const hysl_float_t deltaEta, const int BoucWen_Type,
        const char *Description, BoucWenSurface_t *const Sub) {
    int i;
...
void Substructure_BoucWen (const hysl_float_t DispTdT, BoucWen_t *const Sub, hysl_float_t *const force) {
    ...
    z_new = 1.0;
#endif
    z_new_p = Sub->startPoint;
    z_eval = Sub->startPoint;
    ...
    while (fabs(z_new_p - z_new) > Sub->tolerance && count < Sub->maxIter) {
        ...
        if (Sub->BoucWen_Type == BOUC_WEN_BABER_NOORI) {
            hze_ = deltaDisp * Sub->ko / Sub->Fy * Sub->p * Sub->vs0
                    * exp(
                            -pow(
                                    z_eval * signum(deltaDisp)
                                            - Sub->q
                                                    * pow(1.0 / ((Sub->beta + Sub->gamma) * (Sub->nu0 + Sub->deltaNu * (Sub->e_old - deltaDisp * Sub->ko / Sub->Fy * z_eval * (Sub->alpha - 1.0)))),
                                                            1.0 / Sub->n), 2.0)
                                    / (pow(Sub->lambda - Sub->vs0 * (exp(-Sub->p * (Sub->e_old - deltaDisp * Sub->ko / Sub->Fy * z_eval * (Sub->alpha - 1.0))) - 1.0), 2.0)
                                            * pow(Sub->psi0 + Sub->deltaPsi * (Sub->e_old - deltaDisp * Sub->ko / Sub->Fy * z_eval * (Sub->alpha - 1.0)), 2.0)))
                    * exp(-Sub->p * (Sub->e_old - deltaDisp * Sub->ko / Sub->Fy * z_eval * (Sub->alpha - 1.0))) * (Sub->alpha - 1.0)
                    - Sub->vs0
                            * exp(
                                    -pow(
                                            z_eval * signum(deltaDisp)
                                                    - Sub->q
                                                            * pow(
                                                                    1.0
                                                                            / ((Sub->beta + Sub->gamma)
                                                                                    * (Sub->nu0 + Sub->deltaNu * (Sub->e_old - deltaDisp * Sub->ko / Sub->Fy * z_eval * (Sub->alpha - 1.0)))),
                                                                    1.0 / Sub->n), 2.0)
                                            / (pow(Sub->lambda - Sub->vs0 * (exp(-Sub->p * (Sub->e_old - deltaDisp * Sub->ko / Sub->Fy * z_eval * (Sub->alpha - 1.0))) - 1.0), 2.0)
                                                    * pow(Sub->psi0 + Sub->deltaPsi * (Sub->e_old - deltaDisp * Sub->ko / Sub->Fy * z_eval * (Sub->alpha - 1.0)), 2.0)))
                            * (exp(-Sub->p * (Sub->e_old - deltaDisp * Sub->ko / Sub->Fy * z_eval * (Sub->alpha - 1.0))) - 1.0)
                            * ((2
                                    * (signum(deltaDisp)
                                            - (Sub->deltaNu * deltaDisp * Sub->ko / Sub->Fy * Sub->q * (Sub->alpha - 1.0)
                                                    * pow(1.0 / ((Sub->beta + Sub->gamma) * (Sub->nu0 + Sub->deltaNu * (Sub->e_old - deltaDisp * Sub->ko / Sub->Fy * z_eval * (Sub->alpha - 1.0)))),
                                                            1.0 / Sub->n - 1.0))
                                                    / (Sub->n * (Sub->beta + Sub->gamma)
                                                            * pow(Sub->nu0 + Sub->deltaNu * (Sub->e_old - deltaDisp * Sub->ko / Sub->Fy * z_eval * (Sub->alpha - 1.0)), 2.0)))
                                    * (z_eval * signum(deltaDisp)
                                            - Sub->q
                                                    * pow(1.0 / ((Sub->beta + Sub->gamma) * (Sub->nu0 + Sub->deltaNu * (Sub->e_old - deltaDisp * Sub->ko / Sub->Fy * z_eval * (Sub->alpha - 1.0)))),
                                                            1.0 / Sub->n)))
                                    / (pow(Sub->lambda - Sub->vs0 * (exp(-Sub->p * (Sub->e_old - deltaDisp * Sub->ko / Sub->Fy * z_eval * (Sub->alpha - 1.0))) - 1.0), 2.0)
                                            * pow(Sub->psi0 + Sub->deltaPsi * (Sub->e_old - deltaDisp * Sub->ko / Sub->Fy * z_eval * (Sub->alpha - 1.0)), 2.0))
                                    + (2 * Sub->deltaPsi * deltaDisp * Sub->ko / Sub->Fy * (Sub->alpha - 1.0)
                                            * pow(
                                                    z_eval * signum(deltaDisp)
                                                            - Sub->q
                                                                    * pow(
                                                                            ...
                                                                            1.0 / Sub->n), 2.0))
                                            / (pow(Sub->lambda - Sub->vs0 * (exp(-Sub->p * (Sub->e_old - deltaDisp * Sub->ko / Sub->Fy * z_eval * (Sub->alpha - 1.0))) - 1.0), 2.0)
                                                    * pow(Sub->psi0 + Sub->deltaPsi * (Sub->e_old - deltaDisp * Sub->ko / Sub->Fy * z_eval * (Sub->alpha - 1.0)), 3))
                                    + (2 * deltaDisp * Sub->ko / Sub->Fy * Sub->p * Sub->vs0 * exp(-Sub->p * (Sub->e_old - deltaDisp * Sub->ko / Sub->Fy * z_eval * (Sub->alpha - 1.0)))
                                            * (Sub->alpha - 1.0)
                                            * pow(
                                                    z_eval * signum(deltaDisp)
                                                            - Sub->q
                                                                    * pow(
                                                                            1.0
                                                                                    / ((Sub->beta + Sub->gamma)
                                                                                            * (Sub->nu0 + Sub->deltaNu * (Sub->e_old - deltaDisp * Sub->ko / Sub->Fy * z_eval * (Sub->alpha - 1.0)))),
                                                                            1.0 / Sub->n), 2.0))
                                            / (pow(Sub->lambda - Sub->vs0 * (exp(-Sub->p * (Sub->e_old - deltaDisp * Sub->ko / Sub->Fy * z_eval * (Sub->alpha - 1.0))) - 1.0), 3.0)
                                                    * pow(Sub->psi0 + Sub->deltaPsi * (Sub->e_old - deltaDisp * Sub->ko / Sub->Fy * z_eval * (Sub->alpha - 1.0)), 2.0)));
...
void Substructure_BoucWenSurface (const hysl_float_t DispTdT1, const hysl_float_t DispTdT2, BoucWenSurface_t *const Sub, hysl_float_t *const force1, hysl_float_t *const force2) {
    ...
    fz_new_[0] = 1.0
            + tmp4 * Sub->ko / Sub->Fy
                    * (pow(Sub->z[1], 3.0) * deltaDisp[1] + 2.0 * pow(Sub->z[1], 2.0) * Sub->z[0] * deltaDisp[0] + Sub->z[1] * pow(Sub->z[0], 2.0) * deltaDisp[1] * (Sub->n - 1.0)
                            + pow(Sub->z[0], 3.0) * deltaDisp[0] * Sub->n);
    fz_new_[1] = Sub->z[1] * tmp4 * Sub->ko / Sub->Fy
            * (pow(Sub->z[1], 2.0) * deltaDisp[0] + Sub->z[0] * Sub->z[1] * deltaDisp[1] * (Sub->n - 2.0) + pow(Sub->z[0], 2.0) * deltaDisp[0] * (Sub->n - 1.0));
    fz_new_[2] = Sub->z[0] * tmp4 * Sub->ko / Sub->Fy
            * (pow(Sub->z[0], 2.0) * deltaDisp[1] + Sub->z[0] * Sub->z[1] * deltaDisp[0] * (Sub->n - 2.0) + pow(Sub->z[1], 2.0) * deltaDisp[1] * (Sub->n - 1.0));
    fz_new_[3] = 1.0
            + tmp4 * Sub->ko / Sub->Fy
                    * (pow(Sub->z[0], 3.0) * deltaDisp[0] + 2.0 * pow(Sub->z[0], 2.0) * Sub->z[1] * deltaDisp[1] + Sub->z[0] * pow(Sub->z[1], 2.0) * deltaDisp[0] * (Sub->n - 1.0)
                            + pow(Sub->z[1], 3.0) * deltaDisp[1] * Sub->n);
...
Sub->z_old[0] = Sub->z[0];
Sub->z_old[1] = Sub->z[1];
Sub->DispT[0] = DispTdT1;
Sub->DispT[1] = DispTdT2;
