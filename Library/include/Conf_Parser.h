 ...
 *
 * Routines for reading INI type configuration files. It supports configuration files with section names and
 * therefore it can distinguish two variables of identical name but that are placed in different sections.
...
typedef struct ConfigFile {
     ...
     char **Values;           /*!< \brief Stores the string on the right of the equal sign without any
			       * comments. */
} ConfFile_t;
...
#define MAX_LENGTH 256        /*!< \brief Maximum allowed length of a line. Increase this number for
			       * supporting longer lines. */

/**
 * \brief Allocates space for storing the entries in the configuration file.
 *
 * \param[in] Size Number of Keys and Values entries allowed.  \returns NULL if the allocation of memory
 * failed. A pointer to the allocated memory otherwise.
 *
 * \sa ConfFile.
 */
 ...
 *
 * A configuration file is read and stored in memory for further processing. It assumes a maximum lenght of a
 * line to be \c MAX_LENGTH.  It evaluates, process and finally stores every line in the configuration file
 * and it only stops until either the maximum number of entries is reached or the end of file identifier is
 * found.
 *
 * \param[in] FileName Name of the configuration file.  \param[in,out] CFile Stores the entries of the
 *                     configuration file. On entry only the maximum number of entries, \c Length, is
 *                     referenced.
 *
 * \sa ConfFile_t.
 */
 ...
 *
 * The contents of a line are processed and stored accordingly. The string on the left of the equal sign '\b
 * =' is stored on the \c Key variable while the right string on the \c Value variable. If a comment sign, '\b
 * #' or '\b ;', is found, the rest of the line is ignored. If a entry contains no value, a 0 is assumend.
 *
 * \param[in]  Line  String to be processed.
 * \param[out] Key   On exit contains the string on the right side of the equal sign '\b ='.
 * \param[out] Value On exit contains the string, until a comment sign is found if any, on the left side of
 *                   the equal sign '\b ='.
 ...
 *
 * \pre - A new \c Key and \c Value obtained by evaluating and processing the information of a line by
 * ConfFile_EvalLine() and ConfFile_ProcessLine() respectively.  - \c Entry must be formated as
 ...
 * \param[in,out] CFile Configuration file. On entry only the \c NumEntries variable and the maximum number of
 *                      entries (\c Length) is referenced.
 *
 ...
 * \param[in] CFile Structure with the entries of the configuration file.
 * \param[in] Key   Entry to be found in \c CFile. It should be formated as "SectionName:KeyName".
 * \returns         The integer value assigned to the same position of the \c Key array in the configuration
 *                  file.
 *
 * \sa ConfFile_t.
 */
 ...
 * \param[in] CFile Structure with the entries of the configuration file.
 * \param[in] Key   Entry to be found in \c CFile. It should be formated as "SectionName:KeyName".
 * \returns         The float value assigned to the same position of the \c Key array in the configuration
 *                  file.
 *
 * \sa ConfFile_t.
 */
 ...
 * \param[in] CFile Structure with the entries of the configuration file.
 * \param[in] Key   Entry to be found in \c CFile. It should be formated as "SectionName:KeyName".
 * \returns         The double value assigned to the same position of the \c Key array in the configuration
 *                  file.
 *
 * \sa ConfFile_t.
 */
 ...
 * \returns         The string value assigned to the same position of the \c Key array in the configuration
 *                  file.
 *
 * \sa ConfFile_t.
 */
...
/**
 * \brief Deallocates the dynamically allocated memory in the <tt>ConfFile_t struct</tt>.
 *
 * \pre \c CFile must be properly initialised through ConfFile_Create().
 *
