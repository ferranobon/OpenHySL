 ...
 *
 * Routines for calculating the error force compensation. The routines make use of the BLAS library to perform
 * the linear algebra operations and they support both single and double precision. Sparse BLAS operations are
 * supported through the Intel MKL library.
 */
 ...
 *
 * \f[\vec e^{t+\Delta t} = (\vec f_r^{t + \Delta t} + \vec f_s^{t+\Delta t} + \vec l_i^{t} - (\mathcal M
 * \ddot{\vec u}^{t + \Delta t} + \mathcal C \dot{\vec u}^{t+\Delta t} + \mathcal K u^{t + \Delta t})\f]
 * \f[\vec f_e^{t + \Delta t} = \biggl[\vec e^t + I\Delta t\sum_i^t \vec e^t + \frac{D}{\Delta t} (\vec e^t -
 * \vec e^{t-\Delta t})\biggr]\f]
 *
 * where:
 * - \f$\vec e^t\f$ is the equilibrium error at the end of the time step,
 * - \f$\vec f_e^t\f$ is the compensation force,
 * - \f$f_r^{t+\Delta t}\f$ and \f$f_s^{t+\Delta t}\f$ are the calculated and measured force vectors at time
 *   \f$t + \Delta t\f$,
 ...
 * - and \f$P\f$, \f$I\f$ and \f$D\f$ are the proportional, integral and derivative constants of the PIR error
 *   compensator.
 * 
 * It makes use of BLAS routines to perform the lineal algebra operations. For sparse matrices or matrices in
 * packed storage, the routines ErrorForce_PID_Sp() or ErrorForce_PID_PS() should be used instead.
 *
 * \pre
 ...
 * \param[in]     VelTdT  The velocity vector \f$\dot{\vec u}^{t+\Delta t}\f$.
 * \param[in]     DispTdT The displacement vector \f$\vec u^{t+\Delta t}\f$.
 * \param[in]     fc      The coupling force vector \f$\vec f_c^{t+\Delta t} = \vec f_r^{t + \Delta t} + \vec
 *                        f_s^{t+\Delta t}\f$.
 * \param[in]     LoadTdT The input load vector \f$l_i^t\f$.
 * \param[in]     PID     Values of the PID controller and it contains a proportional \f$P\f$, integral
 *                        \f$I\f$ and derivative \f$D\f$ constants.
 * \param[in,out] fe      The compensation force vector \f$\vec fe^t\f$. As an input, only the size of the
 *                        matrix is referenced, not its elements.
 *
 ...
 */
void ErrorForce_PID( const MatrixVector_t *const Mass, const MatrixVector_t *const Damp,
		     const MatrixVector_t *Stiff, const MatrixVector_t *const AccTdT,
		     const MatrixVector_t *const VelTdT, const MatrixVector_t *const DispTdT,
		     const MatrixVector_t *const fc, const MatrixVector_t *const LoadTdT,
		     const PID_t *const PID, MatrixVector_t *const fe );
 ...
 *
 * \pre
 * - \c Mass, \c Damp and \c Stiff should be symmetrical matrices in packed storage format with the upper
 *   triangular part referenced (lower part in FORTRAN). They should also be properly initialised through the
 ...
 * \sa MatrixVector_t and PID_t.
 */
void ErrorForce_PID_PS( const MatrixVector_t *const Mass, const MatrixVector_t *const Damp,
			const MatrixVector_t *Stiff, const MatrixVector_t *const AccTdT,
			const MatrixVector_t *const VelTdT, const MatrixVector_t *const DispTdT,
			const MatrixVector_t *const fc, const MatrixVector_t *const LoadTdT,
			const PID_t *const PID, MatrixVector_t *const fe );
 ...
 */
void ErrorForce_PID_Sp( const MatrixVector_Sp_t *const Mass, const MatrixVector_Sp_t *const Damp,
			const MatrixVector_Sp_t *const Stiff, const MatrixVector_t *const AccTdT,
			const MatrixVector_t *const VelTdT, const MatrixVector_t *const DispTdT,
			const MatrixVector_t *const fc, const MatrixVector_t *const LoadTdT,
			const PID_t *const PID, MatrixVector_t *const fe );
 ...
 *   compensator.
 * 
 * It makes use of PBLAS routines to perform the lineal algebra operations. For a non-MPI version in general,
 * packed or sparse format, the routines ErrorForce_PID(), ErrorForce_PID_PS(), ErrorForce_PID_Sp() or should
 * be used instead.
 *
 * \pre
 * - All elements of type \c MatrixVector_t must be properly initialised through the PMatrixVector_Create()
 *   routine.
 * - The matrices must be symmetrical and only the upper part will be referenced (lower part in FORTRAN
 *   routines).
 * - The dimensions of the matrices must be the identical.
 ...
 * \param[in]     fc      is the coupling force vector \f$\vec f_c^{t+\Delta t} = \vec f_r^{t + \Delta t} +
 *                        \vec f_s^{t+\Delta t}\f$.
 * \param[in]     LoadTdT is the input load vector \f$l_i^t\f$.
 * \param[in]     PID     Values of the PID controller and it contains a proportional \f$P\f$, integral
 *                        \f$I\f$ and derivative \f$D\f$ constants.
 * \param[in,out] fe      is the compensation force vector \f$\vec fe^t\f$. As an input, only the size of the
 *                        matrix is referenced, not its elements.
 *
 * \post \c fe is the distributed compensation force vector and contains the result of:
 *
 * \f[\vec e^{t+\Delta t} = (\vec f_r^{t + \Delta t} + \vec f_s^{t+\Delta t} + \vec l_i^{t} - (\mathcal M
 * \ddot{\vec u}^{t + \Delta t} + \mathcal C \dot{\vec u}^{t+\Delta t} + \mathcal K u^{t + \Delta t})\f]
 * \f[\vec f_e^{t + \Delta t} = \biggl[\vec e^t + I\Delta t\sum_i^t \vec e^t + \frac{D}{\Delta t} (\vec e^t -
