...
void Compute_NewState_HHT(const MatrixVector_t *const IGain, const MatrixVector_t *const Eff_ForceT, const MatrixVector_t *const In_LoadT,
        const MatrixVector_t *const In_LoadTdT, const MatrixVector_t *const Err_ForceT, MatrixVector_t *const Tempvec, const hysl_float_t Alpha_H,
        MatrixVector_t *const VecTdT_0) {
    int32_t incx = 1;
    int32_t incy = 1;             /* Stride in the vectors */
    ...
    hysl_float_t Alpha = 1.0 + Alpha_H;
...
void Compute_NewState_Zienkiewicz(const MatrixVector_t *const Meff, const MatrixVector_t *const MatA, const MatrixVector_t *const MatB,
        const MatrixVector_t *const DispT, const MatrixVector_t *const DispT0, const MatrixVector_t *const In_LoadT, const MatrixVector_t *const Err_ForceT,
        const MatrixVector_t *const ForceT, const MatrixVector_t *const ForceT0, const hysl_float_t a8, const hysl_float_t a17, const hysl_float_t a18,
    ...
    Alpha = 1.0;
    hysl_float_t Beta = 1.0;
    char uplo = 'L';  /* The lower part (upper part in C) will be used and the upper part (lower part in C) will strictly not be referenced */
    hysl_symv(&uplo, &Tempvec->Rows, &Alpha, MatA->Array, &Tempvec->Rows, DispT->Array, &incx, &Beta, Tempvec->Array, &incy);
    ...
    hysl_symv(&uplo, &Tempvec->Rows, &Alpha, MatB->Array, &Tempvec->Rows, DispT0->Array, &incx, &Beta, Tempvec->Array, &incy);
    /* BLAS: Tempvec = MatA*DispT - MatB*DispT0 + 1/2*beta*Deltat^2*In_LoadT +(1/2 - 2*beta + gamma)*ForceT
    ...
    Alpha = 1.0;
    Beta = 0.0;
    hysl_symv(&uplo, &VecTdT_0->Rows, &Alpha, Meff->Array, &Tempvec->Rows, Tempvec->Array, &incx, &Beta, VecTdT_0->Array, &incy);
}

void Compute_NewState_Zienkiewicz_PS(const MatrixVector_t *const Meff, const MatrixVector_t *const MatA, const MatrixVector_t *const MatB,
        const MatrixVector_t *const DispT, const MatrixVector_t *const DispT0, const MatrixVector_t *const In_LoadT, const MatrixVector_t *const Err_ForceT,
        const MatrixVector_t *const ForceT, const MatrixVector_t *const ForceT0, const hysl_float_t a8, const hysl_float_t a17, const hysl_float_t a18,
        MatrixVector_t *const Tempvec, MatrixVector_t *const VecTdT_0) {
    int32_t incx = 1;
    ...
    hysl_float_t Beta = 1.0;
    char uplo = 'L';  /* The lower part (upper part in C) will be used and the upper part (lower part in C) will strictly not be referenced */
    hysl_spmv(&uplo, &Tempvec->Rows, &Alpha, MatA->Array, DispT->Array, &incx, &Beta, Tempvec->Array, &incy);
    /* BLAS: Tempvec = MatA*DispT + MatB*DispT0 + 1/2*beta*Deltat^2*In_LoadT +(1/2 - 2*beta + gamma)*ForceT
     * + (1/2 + beta - gamma)*ForceT0 = Tempvec - MatB*DispT0 */
    /* Note: MatB has already the minus sign applied */
    hysl_spmv(&uplo, &Tempvec->Rows, &Alpha, MatB->Array, DispT0->Array, &incx, &Beta, Tempvec->Array, &incy);
    /* BLAS: Tempvec = MatA*DispT - MatB*DispT0 + 1/2*beta*Deltat^2*In_LoadT +(1/2 - 2*beta + gamma)*ForceT
     * + (1/2 + beta - gamma)*ForceT0 = Tempvec - MatB*DispT0 + 1/2*beta*Deltat^2*Err_ForceT = Tempvec +
     * a8*Err_ForceT */
    Alpha = a8;
    ...
    Alpha = 1.0;
    Beta = 0.0;
    hysl_spmv(&uplo, &VecTdT_0->Rows, &Alpha, Meff->Array, Tempvec->Array, &incx, &Beta, VecTdT_0->Array, &incy);
}
