...
void Compute_NewState_Zienkiewicz_Sp (const MatrixVector_t *const Meff, const MatrixVector_Sp_t *const MatA, const MatrixVector_Sp_t *const MatB, const MatrixVector_t *const DispT,
        const MatrixVector_t *const DispT0, const MatrixVector_t *const In_LoadT, const MatrixVector_t *const Err_ForceT, const MatrixVector_t *const ForceT, const MatrixVector_t *const ForceT0,
        const hysl_float_t a8, const hysl_float_t a17, const hysl_float_t a18, MatrixVector_t *const Tempvec, MatrixVector_t *const VecTdT_0) {
    ...
    hysl_mkl_csrmv(&trans, &Tempvec->Rows, &Tempvec->Rows, &Alpha, matdescra, MatA->Values, MatA->Columns, MatA->RowIndex, &MatA->RowIndex[1], DispT->Array, &Beta, Tempvec->Array);
    ...
    hysl_mkl_csrmv(&trans, &Tempvec->Rows, &Tempvec->Rows, &Alpha, matdescra, MatB->Values, MatB->Columns, MatB->RowIndex, &MatB->RowIndex[1], DispT0->Array, &Beta, Tempvec->Array);
    /* BLAS: Tempvec = MatA*DispT - MatB*DispT0 + 1/2*beta*Deltat^2*In_LoadT +(1/2 - 2*beta + gamma)*ForceT
