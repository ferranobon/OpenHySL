...
void Rayleigh_Damping_Sp (const MatrixVector_Sp_t *const Mass, const MatrixVector_Sp_t *const Stif, MatrixVector_Sp_t *const Damp, const Rayleigh_t *const Rayleigh) {
...
#pragma omp parallel for
    ...
    hysl_mkl_csradd(&trans, &job, &sort, &Temp.Rows, &Temp.Cols, Temp.Values, Temp.Columns, Temp.RowIndex, &beta, Stif->Values, Stif->Columns, Stif->RowIndex, Damp->Values, Damp->Columns,
            Damp->RowIndex, &Damp->Num_Nonzero, &info);
