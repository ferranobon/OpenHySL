...
const char *Entry_Names[NUM_CHANNELS] = { "Sub-step", "Control displacement actuator 1 [m]", "Control displacement actuator 2 [m]",
        "Control displacement actuator 3 [m]", "Control displacement actuator 4 [m]", "Measured displacement actuator 1 [m]",
        "Measured displacement actuator 2 [m]", "Measured displacement actuator 3 [m]", "Measured displacement actuator 4 [m]",
        "Measured displacement actuator 5 [m]", "Control force actuator 5 [N]", "Measured force actuator 1 [N]", "Measured force actuator 2 [N]",
        "Measured force actuator 3 [N]", "Measured force actuator 4 [N]", "Measured force actuator 5 [N]", "Total vertical force (actuators 4 and 5) [N]",
        "Moment (actuators 4 and 5) [Nm]", "Total shear force (actuators 1, 2 and 3) [N]", };
...
int main(int argc, char **argv) {
    ...
    struct option long_options[] = { { "help", no_argument, 0, 'h' }, { "config-file", required_argument, 0, 'c' }, { 0, 0, 0, 0 } };
    ...
    } else if (((!InitCnt.Use_Sparse && !InitCnt.Read_Sparse) || (InitCnt.Use_Sparse && !InitCnt.Read_Sparse) || (!InitCnt.Use_Sparse && InitCnt.Read_Sparse))
            && InitCnt.Use_Packed) {
    ...
    Algorithm_ReadDataEarthquake(InitCnt.NStep, InitCnt.FileData1, InitCnt.Scale_Factor, AccAll1, VelAll1, DispAll1);
    Algorithm_ReadDataEarthquake(InitCnt.NStep, InitCnt.FileData2, InitCnt.Scale_Factor, AccAll2, VelAll2, DispAll2);
    Algorithm_ReadDataEarthquake(InitCnt.NStep, InitCnt.FileData3, InitCnt.Scale_Factor, AccAll3, VelAll3, DispAll3);
    ...
    while (istep <= InitCnt.NStep) {
        ...
        if (!InitCnt.Use_Sparse && !InitCnt.Use_Packed) {
            EffK_EffectiveForce(&M, &C, &DispT, &VelT, &AccT, &tempvec, InitCnt.a0, InitCnt.a1, InitCnt.a2, InitCnt.a3, InitCnt.a4, InitCnt.a5, &EffT);
        } else if (!InitCnt.Use_Sparse && InitCnt.Use_Packed) {
            EffK_EffectiveForce_PS(&M, &C, &DispT, &VelT, &AccT, &tempvec, InitCnt.a0, InitCnt.a1, InitCnt.a2, InitCnt.a3, InitCnt.a4, InitCnt.a5, &EffT);
        } else if (InitCnt.Use_Sparse) {
#if _SPARSE_
	       EffK_EffectiveForce_Sp( &Sp_M, &Sp_C, &DispT, &VelT, &AccT, &tempvec, InitCnt.a0, InitCnt.a1, InitCnt.a2, InitCnt.a3, InitCnt.a4, InitCnt.a5, &EffT );
#endif
        }
        ...
        if (CNodes.Order >= 1) {
            if (InitCnt.Use_Absolute_Values) {
                Substructure_Substepping(Keinv_c.Array, DispTdT0_c.Array, InitCnt.Delta_t * (hysl_float_t) istep, 0.0, InitCnt.NSubstep, InitCnt.DeltaT_Sub,
                        &CNodes, DispTdT.Array, fcprevsub.Array, fc.Array);
            } else {
                Substructure_Substepping(Keinv_c.Array, DispTdT0_c.Array, InitCnt.Delta_t * (hysl_float_t) istep, AccAll1[istep - 1], InitCnt.NSubstep,
    ...
    /* Free the coupling nodes memory and close sockets if appropriate */
