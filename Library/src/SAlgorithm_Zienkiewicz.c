...
int main( int argc, char **argv ){
     ...
     while ( istep <= InitCnt.NStep ){
	  ...
	  if( CNodes.Order >= 1 ){
	       if( InitCnt.Use_Absolute_Values ){
		    Substructure_Substepping( G_c.Array, DispTdT0_c.Array, InitCnt.Delta_t*(hysl_float_t) istep, 0.0,
					      InitCnt.NSubstep, InitCnt.DeltaT_Sub, &CNodes, DispTdT.Array, fcprevsub.Array,
					      fc.Array );
	       } else {
		    Substructure_Substepping( G_c.Array, DispTdT0_c.Array, InitCnt.Delta_t*(hysl_float_t) istep,
