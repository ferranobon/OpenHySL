...
void CreateDistMatrix( int icntxt, PMatrixVector *const Mat, const int NumRows, const int NumCols, const int BlRows, int const BlCols )
{
	...
	static int myrow, mycol;  /* Variables to store row and column in the process grid */
	static int nprow, npcol;

	static int izero = 0;     /* Zero of type integer. Used by numroc_( ) */
	static int lld, info;
