...
void ADwin_Boot( const int32_t Device_Number, const char *Boot_Path  )
{
      ...
      */
     ADwin_TestVersion( );
     Print_Message( SUCCESS, 1, STRING, "ADwin: Boot successful." );  
}
			  ...
			  const unsigned short int Num_Channels, const char **Chan_Names, const int DataIndex )
{
     ...
     if( Data == NULL ){
	  Print_Message( WARNING, 1, STRING,
			   ...
			   const unsigned short int Num_Channels, const char **Chan_Names, const int DataIndex )
{
     ...
     if( OutFile == NULL ){
	  Print_Message( WARNING, 3, STRING, "ADwin_SaveData_TXT: Could not open ", STRING, FileName, STRING, "." );
     }
