...
int main( int argc, char **argv ){
     ...
     while ( istep <= InitCnt.NStep ){
	  ...
	  if( InitCnt.Order == 961 ){
	       if( VelT.Array[961 - 1] - VelT.Array[47-1] > epsilon ){
		    LoadTdT.Array[961 - 1] = LoadTdT.Array[961 - 1] - Ffs;
	       }
	       if( VelT.Array[961 - 1] - VelT.Array[47-1] < -epsilon ){
		    LoadTdT.Array[961 - 1] = LoadTdT.Array[961 - 1] + Ffs;
	       }
	       if( fabs(VelT.Array[961-1] - VelT.Array[47-1]) < epsilon ){
		    if( LoadTdT.Array[961 - 1] - LoadTdT.Array[47-1] < -Ffst ){
			 LoadTdT.Array[961 - 1] = LoadTdT.Array[961 - 1] + Ffst;
		    }
		    if( LoadTdT.Array[961 - 1] - LoadTdT.Array[47-1] > Ffs ){
			 LoadTdT.Array[961 - 1] = LoadTdT.Array[961 - 1] - Ffst;
		    }
		    if( fabs(LoadTdT.Array[961 - 1] - LoadTdT.Array[47-1]) < Ffst ){
			 LoadTdT.Array[961 - 1] = 0.0;
		    }
	       }
	  }
