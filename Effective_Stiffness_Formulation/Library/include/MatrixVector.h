 ...
 *
 * \pre
 * - All elements of type \c MatVec must be properly initialised through
 * MatrixVector_Create().
 ...
 *
 * It makes use of BLAS and LAPACK routines to perform the lineal algebra operations. For
 * the packed storage and sparse version use the MatrixVector_Add3Mat_PS() and
 * MatrixVector_Add3Mat_Sp() routines respectively.
 * 
 ...
 *
 * A \c MatrixVector_t type is initialised. The routine allocates an amount of memory as a
 * single dimenson array with length \f$L = Rows*Cols\f$. The number of rows and columns
 * is also stored and all elements of the array are initialised and set to 0.0. For
 * symmetric matrices in packed storage or sparse matrices the routines
 * MatrixVector_Create_PS() or MatrixVector_Create_Sp() should be used respectively.
 *
 ...
 * formatted file. It can handle only sparse formats but the output will always be a dense
 * matrix. If a sparse or a packed storage format are desired the routines
 * MatrixVector_FromFile_MM_Sp() or MatrixVector_FromFile_MM_PS() should be used instead.
 *
 ...
 * 
 * A basic linear algebra operation is performed on one of the elements of the matrix or
 * vector. The operation performed is controlled through the variable \c
 ...
 * or \c Divide.
 * \param[in,out] MatVec Matrix or vector to be modified. On entry only the number of
 ...
 * \pre \c MatVec must be properly initialised through MatrixVector_Create().
 *
 * All the elements in \c MatVec.Array are set to the specified value. For a packed
 * storage representation of the matrix the routine MatrixVector_Set2Value() should be
 * used instead. It makes use of BLAS routines to perform the lineal algebra operations.
 *
 * \param[in] Value All elements of the matrix or vector will be set to this value.
 * \param[in,out] MatVec Matrix or vector. On input only the number of rows and columns is
