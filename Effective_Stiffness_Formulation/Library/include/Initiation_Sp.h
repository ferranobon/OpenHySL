 ...
 *
 * It makes use of the BLAS, LAPACK and Sparse BLAS routines from the Intel Math Kernel
 * Library (\cite MKL_2013) to perform the linear algebra operations.
 *
 * \pre 
 * - The gain matrix must be properly initialised through the MatrixVector_Create()
 *   routine.
 * - All elements of type \c MatrixVector_Sp_t must be properly intialised through the
 *   MatrixVector_Create_Sp() routine.
 * - The sparse matrices must in Intel's MKL CSR-\em three \em array \em variation and in
 *   one based index.
 * - The matrices must be symmetrical with only the upper part referenced (lower part in
 *   FORTRAN routines).
 * - The dimensions of the matrices must be the identical.
 * - The maximum number or non zero elements of the proportional viscous damping matrix
 *   must be equal to the number of non zero elements in the stiffness matrix.
 * - The values in scalars must be properly initialised and they depend on the used
 *   formulation (see \cite Dorka_1998).
 *
 * \param[in,out] IGain The gain matrix. As an input, only the size of the matrix is
 *                      referenced, not its elements.
 ...
 *
 * \pre 
 * - The gain matrix must be in packed storage and properly initialised through the
 *   MatrixVector_Create_PS() routine.
 * - All elements of type \c MatrixVector_Sp_t must be properly intialised through the
 *   MatrixVector_Create_Sp() routine
 * - The sparse matrices must in Intel's MKL CSR-\em three \em array \em variation and in
 *   one based index.
 * - The sparse matrices must be symmetrical with only the upper part referenced (lower
 ...
 *                      \c Const.Lambda) respectively.
 *
 * \post \c IGain is a symmetric matrix in packed storage with only the upper part
 *       referenced (Lower part in FORTRAN routines). It contains the result of:
 *       \f[\mathcal{G}^{-1} = \lambda(\alpha\mathcal{M} + \beta\mathcal{C} +
 *       \gamma\mathcal{C})^{-1}\f]
 *
