 ...
 *
 * This file contains the prototypes of those functions involved in creating/destroying sparse matrices and vectors.
 */
 ...
 *
 * Structure designed to represent sparse matrices and vectors within the library. The components of the structure were selected
 * in order to match those of Intel's MKL CSR-\em three \em array \em variation format: \c Values, \c Columns and \c RowIndex in addition to
 * the number of rows and columns. For dense matrix representation, \c MatrixVector_t should be used instead.
 *
 * \sa MatrixVector_t.
 */
typedef struct MatVec_Sp {
     ...
     double *Values;   /*!< \brief A double precision array that contains the non-zero elements of a sparse matrix. The non-zero
 ...
 * - The matrices must in Intel's MKL CSR-\em three \em array \em variation and in one
 * based index.
 ...
 * \f$S(\mathcal X) = X.Rows*X.Cols\f$ is the size of the matrix.
 * - The number of non-zero elements of \f$nnz(\mathcal Y) \geq nnz(\mathcal A + \mathcal
 * B + \mathcal C)\f$.
 *
 ...
 *
 * It makes use of BLAS and Sparse BLAS routines from the Intel Math Kernel Library (\cite
 * MKL_2013) to perform the linear algebra operations. For the full or packed storage
 * cases use the MatrixVector_Add3Mat() or MatrixVector_Add3Mat_PS() routines
 * respectively.
 * 
 * \param[in] MatA Symmetric matrix \f$\mathcal A\f$ with only the upper part referenced
 * (lower part in FORTRAN routines).
 * \param[in] MatB Symmetric matrix \f$\mathcal B\f$ with only the upper part referenced
 * (lower part in FORTRAN routines).
 * \param[in] MatC Symmetric matrix \f$\mathcal C\f$ with only the upper part referenced
 * (lower part in FORTRAN routines).
 ...
 * \param[in] MatY Symmetric matrix \f$\mathcal Y\f$ with only the upper part referenced
 * (lower part in FORTRAN routines). On entry only the dimensions and the number of
 * non-zero elements are referenced.
 *
 ...
 *
 * Allocates the space for the MatrixVector_Sp_t according to the Intel's MKL CSR-\em
 * three \em array \em variation (\cite MKL_2013) format.
 ...
 * \param[in,out] MatVec_Sp Sparse matrix or vector to be initialised. On entry only the
...
/**
 * \brief Counts the number of non-zero elements in the upper triangular part of a
 * symmetric matrix.
 *
 * \param[in] Sym_Matrix Single dimensional array of length \f$L = Rows^2\f$. Only the
 * upper part will be referenced.
 * \param[in] Rows Number of rows.

 * \returns Number of non-zero elements in the upper triangular part of the symmetric
 * matrix.
 */
 ...
 *
 * Initialises a sparse matrix or vector according to the Intel's MKL CSR-\em three \em
 * array \em variation (\cite MKL_2013) format.  For a full or packed storage
 * representation the functions MatrixVector_Create() or Matrixvector_Create_PS() should
 * be used instead.
 *
 ...
 * - \c MatVec_Sp must be properly intialised through the MatrixVector_Create_Sp()
 * routine.
 * - The sparse matrix/vector must in Intel's MKL CSR-\em three \em array \em variation
 * and in one based index.
 ...
 *
 * This routine converts a sparse matrix in Intel's MKL CSR-\em three \em array \em
 * variation (\cite MKL_2013) to a dense format. Currently two operations are available
 * and controlled by \c MatVec_Type. If:
 * 
 * - \f$MatVec_Type = 1\f$ the input matrix is considered to be symmetric and only the
     upper will be considered to be present in the sparse matrix.
 * - \f$MatVec_Type = 0\f$ all the elements of the general matrix/vector are considered
     and transfered to the dense format.
 *
 * \param[in] MatVec_Sp Sparse matrix or vector.
 * \param[in] MatVec_Type Type of the input sparse matrix: Symmetrical
 * (\f$MatVec_Type=1\f$) or general (\f$MatVec_Type = 1\f$).
 * \param[in,out] MatVec Dense matrix or vector. On input only the dimensions are
 * referenced.
 *
...
/**
 * \brief Converts a symmetric sparse matrix in CSR-\em three \em array \em variation into
 * a dense packed storage matrix.
 * 
 * \pre 
 * - \c MatVec_PS must be properly initialised through the MatrixVector_Create_PS()
 *   routine.
 * - \c MatVec_Sp must be properly intialised through the MatrixVector_Create_Sp()
 *   routine.
 * - The sparse matrix is symmetric, with only the upper triangular part present, in
 *   Intel's MKL CSR-\em three \em array \em variation and in one based index.
 * - The dimensions of the matrices/vectors must be the same.
 *
 * This routine converts a sparse matrix in Intel's MKL CSR-\em three \em array \em
 * variation (\cite MKL_2013) to a dense packed storage format.
 *
 * \param[in]     MatVec_Sp Symmetric sparse matrix.
 * \param[in,out] MatVec_PS Dense matrix in packed storage On input only the dimensions
 *                          are referenced.
 *
 * \post \c MatVec_PS is a dense representation in packed storage of the sparse
 * matrix/vector in \c MatVec_Sp. Only the upper part is referenced
 *
 * \sa MatrixVector_t and MatrixVector_Sp_t. 
 */
 ...
 * - \c Operation must be either 0 (symmetric matrix) or 1 (dense matrix/vector).
 *
 * This routine converts a dense matrix/vector into a sparse matrix/vector in Intel's MKL
 * CSR-\em three \em array \em variation (\cite MKL_2013). Currently two operations are
 * available and controlled by \c MatVec_Type. If:
 * 
 * - \f$MatVec_Type = 1\f$ the input matrix is considered to be symmetric and only the
 *   upper will referenced.
 * - \f$MatVec_Type = 0\f$ all the elements of the dense matrix/vector are considered and
 *   transfered to the sparse format.
 *
 ...
 *
 * \post
 * - \c MatVec_Sp contains only the non-zero elements of the dense matrix/vector \c
 * MatVec.
 * - If the dense matrix is symmetric, only the upper triangular part will be explicitly
 * transfered to the sparse matrix, ignoring the lower part.
 * - The sparse matrix/vector is in Intel's MKL CSR-\em three \em array \em variation and
 * in one based index.
 *
 * \sa MatrixVector_t and MatrixVector_Sp_t. 
 */
 ...
 * - \c MatVec_Sp.Array, \c MatVec_Sp.Columns and \c MatVec_Sp.RowIndex no longer point to
 * allocated memory.
 *
 * \sa MatrixVector_t.
 */
 ...
 *
 * \pre
 * - \c MatVec_Sp must have the dimensions specified through
 * MatrixVector_SetRowsCols_Sp().
 * - \c Filename must be in MatrixMarket format and stored in a sparse way.
 *
 * This routine reads a matrix or a vector from a MatrixMarket (\cite MatrixMarket)
 * formatted file. It can handle only sparse formats. If a dense matrix is desired the
 * routine MatrixVector_FromFile_MM() should be used instead.
 *
...
/**
 * \brief Writes a sparse matrix vector into an ASCII file in Intel's MKL CSR-\em three
 * \em array \em variation format.
 *
 * \pre
 * - \c MatVec_Sp is a sparse matrix in Intel's MKL CSR-\em three \em array \em variation
 * format.
 * - \c MatVec_Sp should be properly initialised through MatrixVector_Create_Sp().
 *
 * \param[in] MatVec_Sp is a sparse matrix.
 * \param[in] Filename Name of the desired ASCII file.
 *
 * \post Filename is an ASCII file with the sparse matrix represented in the Intel's MKL
 * CSR-\em three \em array \em variation format.
 */
