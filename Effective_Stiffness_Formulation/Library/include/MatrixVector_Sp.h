 ...
 * perform the linear algebra operations. For the dense version use the MatrixVector_Add3Mat() routine.
 * 
 ...
 * only the dimensions and the number of non-zero elements are referenced.
 *
 ...
 *
 * Allocates the space for the MatrixVector_Sp_t according to the Intel's MKL CSR-\em three \em array \em variation (\cite MKL_2013) format.
 ...
 * \param[in,out] MatVec_Sp Sparse matrix or vector to be initialised. On entry only the dimensios are referenced.
 *
 * \post
 * - \c MatVec_Sp.Values has a length equal to \c nnz.
 * - \c MatVec_Sp.Columns has a length equal to \c nnz.
 ...
 *
 * \param[in] Sym_Matrix Single dimensional array of length \f$L = Rows^2\f$. Only the upper part will
 * be referenced.
 * \param[in] Rows Number of rows.
 ...
 *
 * Initialises a sparse matrix or vector according to the Intel's MKL CSR-\em three \em array \em variation (\cite MKL_2013) format.
 * For a dense representation the function MatrixVector_Create() should be used instead.
 *
 ...
 * - The sparse matrix/vector must in Intel's MKL CSR-\em three \em array \em variation and in one based index.
 ...
 *
 * This routine converts a sparse matrix in Intel's MKL CSR-\em three \em array \em variation (\cite MKL_2013) to a dense
 * format. Currently two operations are available and controlled by \c MatVec_Type. If:
 * 
 * - \f$MatVec_Type = 1\f$ the input matrix is considered to be symmetric and only the upper will be considered to be
 * present in the sparse matrix.
 * - \f$MatVec_Type = 0\f$ all the elements of the general matrix/vector are considered and transfered to the dense format.
 *
 * \param[in] MatVec_Sp Sparse matrix or vector.
 * \param[in] MatVec_Type Type of the input sparse matrix: Symmetrical (\f$MatVec_Type=1\f$) or general (\f$MatVec_Type = 1\f$).
 * \param[in,out] MatVec Dense matrix or vector. On input only the dimensios are referenced.
 *
 ...
 * - \c Operation must be either 0 (symmetric matrix) or 1 (dense matrix/vector).
 *
 * This routine converts a dense matrix/vector into a sparse matrix/vector in Intel's MKL CSR-\em three \em array \em variation
 * (\cite MKL_2013). Currently two operations are available and controlled by \c MatVec_Type. If:
 * 
 * - \f$MatVec_Type = 1\f$ the input matrix is considered to be symmetric and only the upper will referenced.
 * - \f$MatVec_Type = 0\f$ all the elements of the dense matrix/vector are considered and transfered to the sparse format.
 *
 ...
 * - If the dense matrix is symmetric, only the upper triangular part will be explicitly transfered to the sparse matrix, ignoring
 * the lower part.
...
/**
 * \brief Writes a sparse matrix vector into an ASCII file in Intel's MKL CSR-\em three \em array \em variation format.
 *
 * \pre
