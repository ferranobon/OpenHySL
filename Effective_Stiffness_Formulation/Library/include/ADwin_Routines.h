 ...
 *
 * This file contains the prototypes of the routines that involve synchronisation communication
 * or process management of ADwin. 
 */
 ...
 * 
 * \warning The graphical interface written in QT4 is recomended to manage ADwin. This function is
 * deprecated.
 * 
 ...
 *
 * \post The status of the process is displayed and the program will terminate if the process is
 * not running.
 *
 * \sa ADwin_Boot().
 */
 ...
 * - When loading a process, PName should point to the ADwin binary file specifically compiled
 * for the ADwin processor that will be run on.
 *
 * Depending on the value of the \c dowhat variable the process will be loaded (\f$ dowhat = 1\f$),
 * started (\f$ dowhat = 2\f$) or stopped (\f$ dowhat = 3\f$). In the case of loading a process, only
 * the \c PName variable is referenced, while in the other cases only the \c PNum variable is used.
 *
 * \param[in] PName Binary file to be loaded into ADwin. Only referenced if \f$dowhat = 1\f$.
 * \param[in] PNum Process number to be started or stopped. Only referenced if \f$dowhat = 2\f$ or
 * \f$dowhat = 3\f$.
 * \param[in] dowhat identifies the operation to be performed. Load (\f$ dowhat = 1\f$), started
 * (\f$ dowhat = 2\f$) or stop (\f$ dowhat = 3\f$) a process.
 *
 ...
 *
 * An array of the specified length is sent into the ADwin system and will overwrite any data that is
 * present into the pointed data array in ADwin.
 *
 * \pre
 ...
 *
 * This routine communicates with ADwin to perform the sub-stepping process in ADwin. It does so by sending the explicit displacement
 * to the ADwin system. It waits a certain amount of time before retrieving the data from the ADwin system. This is done in order to
 * avoid overloading the system with network requests when the system is not yet ready. A good time to get the data from ADwin is:
 *
 * \f[Time_To_Wait = \frac{Round Trip Time}{0.9}\f]
 *
 * Note that this value has to be coherent also with when ADwin measures the last coupling force from the speciment.
 *
 * This routine relies on a synchronization variable when communicating with ADwin.
 * - When sending the data to ADwin, the first value of the array should be \c 1.0. This indicates the running process in ADwin that a
 * new displacement value is available.
 * - If \f$ADwinReady = 0\f$ the sub-stepping process is
 * not finished and therefore the last coupling force is not yet measured. The routine will keep asking ADwin for the last coupling force
 * until the first received value from ADwin is equal to -1.0. When this happens the substepping process is finished.
 *
 * \pre
 * - ADwin must be successfully booted and the substepping process must be running.
 * - The sub-stepping process in ADwin should expect the new displacement vectors to be set in array 2. The first value should be a 1.0 in order
 * to indicate that a new value is available.
 * - The sub-stepping process in ADwin should save the new displacement and force values on array 3. When the values are reaady (the sub-stepping
 * process is finished) the first value should be a -1.0.
 ...
 *
 * The data stored in the specified array ADwin is read and stored into a HDF5 file. The dataset will have
 * as many columns as the specified number of chanels and as many rows as \f$N_{steps}\cdot N_{substeps}\f$.
 *
 * \pre
 ...
 *
 * The data stored in the specified array ADwin is read and stored into the desired ASCII file with
 * as many columns as the specified number of chanels and as many rows as \f$N_{steps}\cdot N_{substeps}\f$.
 *
 * \pre
 * - ADwin must be properly booted and the substructure test must be finished.
