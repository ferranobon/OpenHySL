...
/**
 * \brief Computes the proportional viscous damping matrix using Rayleigh damping. Packed
 * storage version
 *
 * This routine calculates the proportional viscous damping matrix using Rayleigh Damping
 * and packed storage. The operation performed is (see \cite Clough 1975 p. 234):
 *
 * \f[\mathcal{C} = \alpha\mathcal{M} \cdot \beta \mathcal{K}\f]
 *
 * where:
 ...
 * - \f$\mathcal{M}\f$ is the mass matrix,
 * - \f$\mathcal{K}\f$ is the stiffness matrix,
 * - and \f$\alpha\f$ and \f$beta\f$ are the parameters that multiply the mass and
 * stiffness matrices respectively.
 * 
 * It makes use of BLAS routines to perform the linear algebra operations.
 *
 * \pre 
 * - All elements of type \c MatrixVector_t must be properly intialised through the
 ...
 * \param[in] Mass     The mass matrix \f$\mathcal{M}\f$.
 * \param[in] Stiff    The stiffness matrix \f$\mathcal{K}\f$.
 * \param[in,out] Damp The proportional viscous damping matrix \f$\mathcal{C}\f$. As an
 *                     input, only the size of the matrix is referenced.
 * \param[in] Rayleigh Contains the values of the proportional coefficients \f$\alpha\f$
 *                     (\c Rayleigh.Alpha) and \f$\beta\f$ (\c Rayleigh.Beta).
 *
 * \post \c Damp is a symmetric matrix in packed storage. It contains the result of:
 *       \f[\mathcal{C} = \alpha \mathcal{M} \cdot \beta \mathcal{K}\f]
 *
 * \sa MatrixVector_t and Rayleigh_t.
 */
 ...
 * - and \f$\alpha\f$, \f$beta\f$, \f$\gamma\f$ and \f$\lambda\f$ are the parameters that
 *   multiply the mass, damping, stiffness and matrices and the result of the matrix
 ...
 * - The dimensions of the matrices must be the identical.
 * - The values in scalars must be properly initialised and they depend on the used
 *   formulation (see \cite Dorka_1998).
 *
 * \param[in,out] IGain The inverse of the gain matrix. As an input, only the size of the
 ...
 * \param[in]     Const Scalars that multiply the mass (\f$\alpha\f$), \c Const.Aplha),
 *                      damping (\f$\beta\f$), \c Const.Beta), stiffness (\f$\gamma\f$),
 *                      \c Const.Gamma) matrices and the inverted matrix (\f$\lambda\f$),
 *                      \c Const.Lambda) respectively.
 *
 * \post \c IGain is a symmetric matrix in general storage with only the upper part
 *       referenced (Lower part in FORTRAN routines). It contains the result of:
 *       \f[\mathcal{G}^{-1} = \lambda(\alpha\mathcal{M} + \beta\mathcal{C} +
 *       \gamma\mathcal{C})^{-1}\f]
 *
 ...
 * - All elements of type \c MatrixVector_t must be properly intialised through the
 *   MatrixVector_Create_PS() routine.
 * - The matrices must be symmetrical and in packed storage. The upper triangular part
 *   (lower triangular part in FORTRAN) must be present.
 * - The dimensions of the matrices must be the identical.
 * - The values in scalars must be properly initialised and they depend on the used
 *   formulation (see \cite Dorka_1998).
 *
 * \param[in,out] IGain The inverse of the gain matrix in packed storage. As an input,
 *                      only the size of the  matrix is referenced.
 * \param[in]     Mass  The mass matrix.
 * \param[in]     Damp  The proportional viscous damping matrix.
 * \param[in]     Stiff The stiffness matrix.
 * \param[in]     Const Scalars that multiply the mass (\f$\alpha\f$), \c Const.Aplha),
 ...
 *                      \c Const.Lambda) respectively.
 *
 * \post \c IGain is a symmetric matrix in packed storage with only the upper part (lower
 *       part in FORTRAN). It contains the result of: \f[\mathcal{G}^{-1} =
