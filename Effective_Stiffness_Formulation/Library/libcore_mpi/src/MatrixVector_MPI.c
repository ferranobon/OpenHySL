			   ...
			   int const BlCols, PMatrixVector_t *const MatVec )
{
     ...
     MatVec->LocalSize.Row = numroc_( &MatVec->GlobalSize.Row, &MatVec->BlockSize.Row, &myrow, &izero,
				      &nprow );
     MatVec->LocalSize.Col = numroc_( &MatVec->GlobalSize.Col, &MatVec->BlockSize.Col, &mycol, &izero,
				      &npcol );
     lld = Max( 1, MatVec->LocalSize.Row );
	
     descinit_( MatVec->Desc, &MatVec->GlobalSize.Row, &MatVec->GlobalSize.Col, &MatVec->BlockSize.Row,
		&MatVec->BlockSize.Col, &izero, &izero, &icntxt, &lld, &info );
     if ( info < 0 ){
	  Print_Header( ERROR );
				  ...
				  PMatrixVector_t *const MatVec )
{
     ...
     infog2l_( &GRowIndex, &GColIndex, MatVec->Desc, &nprow, &npcol, &myrow, &mycol, &LRowIndex, &LColIndex,
