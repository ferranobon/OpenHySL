				  ...
				  PMatrixVector_t *const AccTdT_Corr )
{
     ...
     pdcopy_( &Tempvec->GlobalSize.Row, In_LoadT->Array, &ione, &ione, In_LoadT->Desc, &incx,
	      Tempvec->Array, &ione, &ione, Tempvec->Desc, &incy );
     /* PBLAS: tempvec = In_LoadT + fc = tempvec + fc */
     pdcopy_( &Tempvec->GlobalSize.Row, fc->Array, &ione, &ione, fc->Desc, &incx,
	      Tempvec->Array, &ione, &ione, Tempvec->Desc, &incy );

     /* PBLAS: AccTdT_Corr = MInv*(In_LoadT + fc) = MInv*Tempvec */
     pdsymv_( &uplo, &Tempvec->GlobalSize.Row, &Alpha, MInv->Array, &ione, &ione, MInv->Desc, 
