				   ...
				   PMatrixVector_t *const CoupForce )
{
     ...
     for( i = 0; i < CNodes->Order; i++ ){
	  if ( rank == 0 && Cblacs_pnum( CoupForce->Desc[1], ILoc_CoupForce->RowProcess[i], ILoc_CoupForce->ColProcess[i] ) == 0 ){
	       ...
	       if ( rank == Cblacs_pnum( CoupForce->Desc[1], ILoc_CoupForce->RowProcess[i], ILoc_CoupForce->ColProcess[i] ) ){
		    MPI_Send( &CoupForce->Array[ILoc_CoupForce->LRowIndex[i] - 1], 1, MPI_HYSL_FLOAT, 0,
			      MPI_FORCE, Comm );
	       } else if ( rank == 0 ){
		    MPI_Recv( &Recv[2*CNodes->Order + i], 1, MPI_HYSL_FLOAT,
			      Cblacs_pnum( CoupForce->Desc[1], ILoc_CoupForce->RowProcess[i], ILoc_CoupForce->ColProcess[i] ),
			      MPI_FORCE, Comm, &Status );
	       }
	  }
     }
     ...
     for( i = 0; i < CNodes->Order; i++ ){
	  if ( rank == 0 && Cblacs_pnum( VecTdT->Desc[1], ILoc_VecTdT->RowProcess[i], ILoc_VecTdT->ColProcess[i] ) == 0 ){
	       ...
	       if ( rank == 0 ){
		    MPI_Send( &Recv[i], 1, MPI_HYSL_FLOAT, Cblacs_pnum( VecTdT->Desc[1], ILoc_VecTdT->RowProcess[i], ILoc_VecTdT->ColProcess[i] ),
			      MPI_NEW_STATE, Comm );
	  ...
	  if ( rank == 0 && Cblacs_pnum( CoupForcePrev->Desc[1], ILoc_CoupForcePrev->RowProcess[i], ILoc_CoupForcePrev->ColProcess[i] ) == 0 ){
	       ...
	       if ( rank == 0 ){
		    MPI_Send( &Recv[1 + i], 1, MPI_HYSL_FLOAT, Cblacs_pnum( CoupForcePrev->Desc[1], i, i),
			      MPI_PREV_FORCE, Comm );
	  ...
	  if ( rank == 0 && Cblacs_pnum( CoupForce->Desc[1], ILoc_CoupForce->RowProcess[i], ILoc_CoupForce->ColProcess[i] ) == 0 ){
	       ...
	       if ( rank == 0 ){
		    MPI_Send( &Recv[2*1 + i], 1, MPI_HYSL_FLOAT, Cblacs_pnum( CoupForce->Desc[1], ILoc_CoupForce->RowProcess[i], ILoc_CoupForce->ColProcess[i] ),
			      MPI_FORCE, Comm );
