			    ...
			    const float *const Data_To_Send, float *Data_To_Receive, const int Order )
{
     ...
     if ( WhatToDo == 0 ){
	  ...
	  if ( recv( Socket, Receive_Buffer, MAXBUFLEN, 0 ) <= 0 ){
	       ...
	       memcpy( Data_To_Receive, Receive_Buffer + 2*sizeof(int), sizeof (float)*Order );
	  }
	  ...
	  if ( recv( Socket, Receive_Buffer, MAXBUFLEN, 0 ) <= 0 ){
	       ...
	       memcpy( &Time, pos, sizeof (float) );
	       pos = pos + sizeof( float );
	       memcpy( Data_To_Receive, pos, sizeof (float)*Order );
