				...
				MatrixVector *const Eff_Force )
{
     ...
     Alpha = 1.0; Beta = 0.0;
     dsymv_( &uplo, &Tempvec->Rows, &Alpha, Mass->Array, &Tempvec->Rows, Tempvec->Array, &incx, &Beta, Eff_Force->Array, &incy );
     ...
     Alpha = 1.0; Beta = 1.0;
     dsymv_( &uplo, &Tempvec->Rows, &Alpha, Damp->Array, &Tempvec->Rows, Tempvec->Array, &incx, &Beta, Eff_Force->Array, &incy );
		     ...
		     MatrixVector *const Tempvec, MatrixVector *const Disp0 )
{
     ...
     Alpha = 1.0;
     dsymv_( &uplo, &Tempvec->Rows, &Alpha, Keinv->Array, &Tempvec->Rows, Tempvec->Array, &incx, &Beta, Disp0->Array, &incy );
}
